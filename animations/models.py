...
class AnimationModel(Model):
    name = CharField(
        max_length=100,
        help_text="Enter a name of the animation",
    )
    width = IntegerField()
    height = IntegerField()
...
class ObjectModel(Model):
    name = CharField(
        max_length=100,
        help_text="Enter a name of the object",
    )
    ...
    def stateIn(self, start, frame_id):
        state = ObjectState(
            self.x_coord, self.y_coord, self.width, self.height, self.active
        )
    ...
    class Meta:
        constraints = [
            CheckConstraint(
                check=Q(type__in=ObjectType.values), name="valid_objcet_type"
            )
        ]
...
class ObjectAnimationModel(Model):
    ...
    class Meta:
        constraints = [
            CheckConstraint(
                check=Q(type__in=AnimationType.values), name="valid_animation_type"
            )
        ]
